...
class DripsTestCase(TestCase):
    ...
    def test_apply_annotations_with_count(self):
        ...
        self.assertEqual(
            list(
                qs.query.aggregate_select.keys()),
    ...
    def message(self):
        if not self._message:
            email = mail.EmailMessage(
                self.subject, self.plain, self.from_email, [
                    self.user.email])
            self._message = email
...
class CustomMessagesTest(TestCase):
    ...
    def test_custom_added_and_used(self):
        ...
        self.assertEqual(1, len(mail.outbox))
        email = mail.outbox.pop()
        # In this case we did specify the custom key, so message should be of
        # custom type.
